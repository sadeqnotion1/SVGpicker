...
{#if chosen}
  <div class="chosen">
    <div class="chosen-preview">
      {#if chosenSvg}{@html chosenSvg}{:else}<img src={chosen.svgUrl} alt={chosen.name} />{/if}
    </div>
    <div class="chosen-body">
      <div class="chosen-name">{chosen.library} · {chosen.name}</div>
      <div class="copy-row">
        <button on:click={() => copy(chosen.icon, "name")}>Copy name</button>
        <button on:click={() => chosenSvg && copy(chosenSvg, "svg")} disabled={!chosenSvg}>Copy SVG</button>
        <button on:click={() => copy(chosen.svgUrl, "url")}>Copy URL</button>
      </div>
      {#if copied}<span class="copied">Copied {copied}!</span>{/if}
    </div>
  </div>
{/if}
      ...
      <div class="icons">
        {#each items as m (m.icon)}
          <button
            ...
            on:click={() => choose(m)}
          >
            <img src={m.svgUrl} alt={m.name} loading="lazy" />
          </button>
        {/each}
      </div>
    </div>
...
<style>
  ...
  .copied { font-size: 12px; color: var(--accent-2); margin-left: 4px; }
